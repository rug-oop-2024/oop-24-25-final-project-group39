...
def show_results():
    if check_inputs is False:
        ...
        if st.button("Compute results"):
            ...
            calculating.empty()
            for i in range(len(chosen_metrics)):
                st.write(f"{chosen_metrics[i]}: {pipeline._metrics_results[i][1]:.2f}")
        st.header("Save Pipeline")
        name_pipeline = st.text_input("Enter a name for this pipeline")
        if st.button("Save Pipeline"):
            artifact_pipeline = pipeline.to_artifact(name_pipeline)
...
new_feature_list = [feature for feature in feature_list if feature not in input_features]
